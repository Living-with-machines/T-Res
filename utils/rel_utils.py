...
# TODO/typing: is ``cursor`` of type ``sqlite3.Cursor``? If so, we should add it.
def get_db_emb(
    cursor, mentions: List[str], embtype: Literal["word", "entity", "snd"]
    ...
    Arguments:
        ...
        embtype (Literal["word", "entity", "snd"]): The type of embedding to
            ...
            ``"snd"``. If it is set to ``"word"``, we use Wikipedia2Vec word
            embeddings, if it is set to ``"entity"``, we use Wikipedia2Vec
            entity embeddings, and if it is set to "snd", we use glove word
            embeddings.
...
def prepare_initial_data(df: pd.DataFrame, context_len: Optional[int] = 100) -> dict:
    ...
    ..
        TODO: Looks like the keyword argument context_len isn't used in
        this function, and can be scrapped.
    """
    dict_mentions = dict()
...
def prepare_rel_trainset(
    ...
    Arguments:
        df (pandas.DataFrame): The pandas DataFrame containing the prepared
            dataset.
        mylinker (geoparser.linking.Linker): The Linking object.
