...
embeddings = np.load('/resources/develop/mcollardanuy/toponym-resolution/experiments/outputs/embeddings/embeddings.npy')
with open('/resources/develop/mcollardanuy/toponym-resolution/experiments/outputs/embeddings/wikidata_ids.txt') as fr:
...
relevant_classes = {"Q6256": "country",
                     "Q5107": "continent",
...
model_rd = pipeline("feature-extraction", 
                    model=base_model_path, 
                    tokenizer=base_model_path)

# Place of publication to Wikidata QID
places_of_publication = {"Ashton-under-Lyne": "Q659803",
                         "Manchester": "Q18125",
                         "Dorchester": "Q503331",
                         "Poole": "Q203349",
                         "New York": "Q60"}
...
def assign_closest_class(avg_embedding):
    ...
    if not avg_embedding is None:
        ...
        for rce in relevant_classes_embeddings:
            dict_class_similarity[rce] = 1 - spatial.distance.cosine(avg_embedding, relevant_classes_embeddings[rce])
...
def select(cands, method, mylinker):
    if cands:
        if "mostpopular" in method:
            link, score, other_cands = most_popular(cands,type=method,mylinker=dict())
    ...
    for candidate in cands:
        ...
        if wikidata_cands:
            ...
            for i in range(len(model_geoscope.classes_)):
                mention_data[model_geoscope.classes_[i]] = np.array(model_geoscope.predict_proba([mention_emb]))[0][i]
            for i in range(len(model_class.classes_)):
                mention_data[model_class.classes_[i]] = np.array(model_class.predict_proba([mention_emb]))[0][i]
            ...
            cands_df.loc[:,'class_emb'] = cands_df.loc[:,:].apply(lambda x: find_avg_node_embedding(x["wqid_cand"]), axis=1)
            cands_df.loc[:,'candidate_classtype'] = cands_df.loc[:,:].apply(lambda x: assign_closest_class(x["class_emb"]), axis=1)
            cands_df.loc[:, 'candidate_geoscope'] = cands_df.loc[:, :].apply(lambda x: get_geoscope_from_publication(mylinker["metadata"]["place"], x["wqid_cand"]), axis=1)
            cands_df['candidate_relevance_norm'] = (cands_df['candidate_relevance'] - min(cands_df['candidate_relevance'])) / (max(cands_df['candidate_relevance']) - min(cands_df['candidate_relevance']))
            ...
            cands_df["weight"] = cands_df[["weight_classtype", "weight_geoscope", "candidate_relevance_norm", "candrank_confidence"]].mean(axis=1)
        
            best_candidate, score = tuple(cands_df.sort_values(by=["weight"], ascending=False).iloc[0][["wqid_cand", "weight"]])
...
def most_popular(cands, type, mylinker):
    keep_most_popular = ""
    keep_highest_score = 0.0
    all_candidates = []
    for candidate in cands:
