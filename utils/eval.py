...
#### Eval function to assess impact of candidate selection step
...
def eval_selection(true_mentions_sents,trues,preds):
    ...
    for s in range(len(true_mentions_sents)):
        true_sent = true_mentions_sents[s]
        for mention in true_sent:
            start_offset = mention['start_offset']
            end_offset = mention['end_offset']
            ent_id = trues[s][start_offset][2]
            if "B-Q" in ent_id:
                ent_id = ent_id[2:]
                pred_cand_ents = {}
                for x in range(start_offset,end_offset+1):
                    if len(preds[s][x])>3:
                        pred_cand_ents = pred_cand_ents | preds[s][x][3].keys()
                tot +=1
                if ent_id in pred_cand_ents:
                    pres +=1

    cand_sel_score = pres/tot
    return cand_sel_score
