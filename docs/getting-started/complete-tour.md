...
-   `load_from_hub`: it indicates whether to load a pre-trained NER model. If it is set to `False`, the Recogniser will be prepared to train a new model, unless the model already exists.
...
-   `train_dataset` and `test_dataset`: the path to the train and test data sets necessary for training the NER model. You can find more information about the format of this data in the "[Resources and directory structure](resources.md)" page in the documentation.
...
In order to use the Ranker and the Linker, we need a knowledge base, a gazetteer. T-Res uses a gazetteer which combines data from Wikipedia and Wikidata. See how to obtain the Wikidata-based resources in the "[Resources and directory structure](resources.md)" page in the documentation.
...
Note that `resources_path` should contain the path to the directory where the Wikidata- and Wikipedia-based resources are stored, as described in the "[Resources and directory structure](resources.md)" page in the documentation.
...
To train a DeezyMatch model from scratch, using an existing string pairs dataset, you will need to have the following ``resources`` file structure (as described in the "[Resources and directory structure](resources.md)" page in the documentation):
...
To train a DeezyMatch model from scratch, including generating a string pairs dataset, you will need to have the following `resources` file structure (as described in the "[Resources and directory structure](resources.md)" page in the documentation):
...
-   `rel_params`: set of parameters specific to the `reldisamb` method:
    ...
    -   `data_path`: Path to the dataset file `linking_df_split.tsv` used for training a model (see information about the dataset in the "[Resources and directory structure](resources.md)" page in the documentation).
    -   `training_split`: Column from the `linking_df_split.tsv` file that indicates which documents are used for training, development, and testing (see more information about this in the "[Resources and directory structure](resources.md)" page in the documentation).
    -   `db_embeddings`: cursor for the embeddings database (see more information about this in the "[Resources and directory structure](resources.md)" page in the documentation).
...
Note that if the model already exists and `overwrite_training` is set to `False`, the training will be skipped, even if you call the `train()` method. The training will also be skipped if the Linker is instantiated for `mostpopular`.
