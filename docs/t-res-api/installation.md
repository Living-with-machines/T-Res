...
4.  Add a section to the docker-compose.yml, updating the service name, image and labels as follows:
    ...
    >
    > <YOUR_CONFIG_NAME>:
    > :   image: <YOUR_CONFIG_NAME>_image:latest restart: always expose:
    >     - 80 volumes:
    ...
    >     -   traefik.http.middlewares.test-stripprefix-rwop.stripprefix.prefixes=/v2/[t-res]()<YOUR_CONFIG_NAME>
    >
    >     - traefik.http.routers.<YOUR_CONFIG_NAME>_router.middlewares=test-stripprefix-rwop command: ["poetry", "run", "uvicorn", "app:app", "--proxy-headers", "--host", "0.0.0.0", "--port", "80", "--root-path", "/v2/t-res_deezy_reldisamb-wpubl-wmtops"]
