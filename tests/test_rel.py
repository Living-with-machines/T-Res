...
lwm_original_df = lwm_original_df
lwm_processed_df = lwm_processed_df
...
for method in methods:
    ...
    preds = mylinker.perform_linking(lwm_processed_df, lwm_original_df, "test")[
        "pred_wqid"
    ].tolist()
    no_nil = [x for x in preds if x != "NIL"]

    print(method, "no_nil_ratio:", len(no_nil) / len(preds))
