...
datasets = ["lwm", "hipe"]
approaches = [{"ner_model_id": "lwm", # LwM baseline
...
for dataset in datasets:
    for approach in approaches:
        ...
        pred = '../experiments/outputs/results/' + dataset + '/' + approach_name + '_bundle2_en_1.tsv'
        ...
        for ne_tag in ne_tags:
            for setting in settings:
                for measure in measures:
                    overall_results_nerc[setting + measure] = round(ner_score['NE-COARSE-LIT']['TIME-ALL']['LED-ALL'][ne_tag][setting][measure], 3)
        ...
        for ne_tag in ne_tags:
            for setting in settings:
                for measure in measures:
                    if setting == "strict":
                        overall_results_nel[setting + measure] = round(linking_score[1]['NEL-LIT']['TIME-ALL']['LED-ALL'][ne_tag][setting][measure], 3)
