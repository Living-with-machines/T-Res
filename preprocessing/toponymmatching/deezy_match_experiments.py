...
train_df = pd.read_csv("../../experiments/outputs/data/lwm/linking_df_train_cands_deezymatch.tsv", sep="\t")
...
for i, row in train_df.iterrows():
    if row["wkdt_qid"] in wikidata_to_mentions:
        ...
        candidates += [x for x in potential_cands if SequenceMatcher(None, row["mention"], x).ratio() > 0.8]
...
list_pooling_modes = ["hstates_layers_simple", "hstates_layers", "hstates_l2_distance", "average", "max"]
    ...
    for pooling_mode in list_pooling_modes:
        for dropout in list_dropouts:
            for learning_rate in list_learning_rates:
                ...
                with open('./experiments/inputs/input_dfm_char_' + str(experiment_id) + '.yaml', 'w') as outfile:
                ...
                if not Path("./experiments/models/" + dm_model + '/' + dm_model + '.model').is_file():
                    # train a new model
                    dm_train(input_file_path="./experiments/inputs/input_dfm_char_" + str(experiment_id) + ".yaml",
                ...
                if not Path("./experiments/candidate_vectors/candidates_" + dm_model + "/embeddings/").is_dir():
                    start_time = time.time()
                    dm_inference(input_file_path="./experiments/models/" + dm_model + "/input_dfm_char_" + str(experiment_id) + ".yaml",
                                    dataset_path="./experiments/datasets/candidates.txt", 
                                    pretrained_model_path="./experiments/models/" + dm_model + "/" + dm_model + ".model", 
                                    pretrained_vocab_path="./experiments/models/" + dm_model + "/" + dm_model + ".vocab",
                                    inference_mode="vect",
                ...
                if not Path("./experiments/query_vectors/queries_" + dm_model + "/embeddings/").is_dir():
                    start_time = time.time()
                    dm_inference(input_file_path="./experiments/models/" + dm_model + "/input_dfm_char_" + str(experiment_id) + ".yaml",
                ...
                candidates_pd = \
                    candidate_ranker(query_scenario="./experiments/combined/queries_" + dm_model,
                ...
                for i, row in train_df.iterrows():
                    if row["mention"] in list(candidates_pd["query"]):
                        returned_candidates = candidates_pd[candidates_pd["query"] == row["mention"]].iloc[0].faiss_distance
                        print(row["mention"])
                ...
                with open("./experiments/results/" + dm_model + ".txt", "w") as fw:
                    ...
                    fw.write("Accuracy:" + str(len(correct)/(len(correct)+len(incorrect))) + "\n")
                    fw.write("Correct distance:" + str(correct_distance) + "\n")
                    fw.write("Incorrect distance:" + str(incorrect_distance) + "\n")
                    fw.write("Distance accuracy:" + str(correct_distance/(correct_distance+incorrect_distance)) + "\n")